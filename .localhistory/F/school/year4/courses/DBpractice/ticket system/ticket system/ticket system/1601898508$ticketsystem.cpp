...
DWORD WINAPI SellTicketThreadProc(LPVOID lpParameter)
{
	...
	while (Info->biz->GetBalanceNum() > 0 && Info->service) {
		...
		if (Info->biz->GetBalanceNum() > 0) {
			...
			QString str = "No.";
			str.append(QString(Info->win_no));
			str.append(" window has sold a ticket: ");
			str.append(QString(ticketno));
			str.append(".\n");
			...
			if (myfile.open(QFile::WriteOnly))//注意WriteOnly是往文本中写入的时候用，ReadOnly是在读文本中内容的时候用，Truncate表示将原来文件中的内容清空
			{
